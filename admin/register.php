<?php 
	...
	if (!empty($_POST['user']) && !empty($_POST['pass']) && !empty($_POST['language']) && !empty($_POST['company'])
		...
		if (mysql_num_rows($testRes)) $ERROR = "Username already exists: ".$user;
			...
			if (!mkdir("../".$SETTINGS['USER_DIR']."/".$user)) die("Unable to create directory for user!");
			//if (!chmod($SETTINGS['USER_DIR']."/".$user, 0777)) die("Unable to make user directory writable!");
			
			$res = mysql_query("INSERT INTO `login` (`user`, `pass`, `Language`, `Company`, `Name`, `Telephone`, `Address`, `PostCode`, `Town`, `Country`, `Date`) VALUES( '".$user."', '".md5($pass)."', '".$language."', '".$company."', '".$name."', '".$telephone."', '".$address."', '".$PostCode."', '".$town."', '".$country."', '".$date."'  )");
...
function CreateUser() 
{
	...
	$.post('register.php?action=do', formdata, function(data) {
  	ShowAlert("Info", data);
	if (data == "New user created successfully!") window.location.reload();
});
return;
}
</script>

<div id="rcont" style="margin:0 auto; width:450px;">

<h2>Add New User</h2>

<table>
<form action="dummy" method="get" id="newuserform" name="newuserform">
<tr><td width="150">
...
<tr><td>
<label for="language">Language:</label></td><td width="250">  <select name="language" id="language">
...
<tr><td>
<label for="date">Date:</label></td><td width="250"><input type="text" name="date" id="date" size="40" value="<?php echo $_POST['date'] ? $_POST['date']: date('m/d/Y'); ?>" />
</td></tr>
</form>
<tr><td colspan="2" align="right"><button id="bcreate" onClick="CreateUser()">Create</button></td></tr>
</table>
